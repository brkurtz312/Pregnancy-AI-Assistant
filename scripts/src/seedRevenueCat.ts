...
// One-time, non-consumable "Full Pregnancy Pass". One-time Play Store products
...
const ENTITLEMENT_DISPLAY_NAME = "Full Pregnancy Pass";

const OFFERING_IDENTIFIER = "default";
const OFFERING_DISPLAY_NAME = "Default Offering";

// $rc_lifetime is RevenueCat's predefined package type for a one-time,
// non-consumable lifetime unlock.
const PACKAGE_IDENTIFIER = "$rc_lifetime";
const PACKAGE_DISPLAY_NAME = "Full Pregnancy Pass";

// Pricing configuration (amount in micros = dollars * 1,000,000). $19.99.
const PRODUCT_PRICES = [{ amount_micros: 19990000, currency: "USD" }];
...
async function seedRevenueCat() {
  ...
  const ensureProductForApp = async (
    ...
    // Display names must be unique across products, so qualify per store.
    const body: CreateProductData["body"] = {
      store_identifier: productIdentifier,
      app_id: targetApp.id,
      type: "non_consumable",
      display_name: `${PRODUCT_DISPLAY_NAME} (${label})`,
    };
