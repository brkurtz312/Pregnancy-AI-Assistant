...
export function ResultsDisplay({ results }: ResultsDisplayProps) {
  ...
  const currentWeekData = getWeeklyDevelopment(
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card className="col-span-1 md:col-span-2 border-primary/20 shadow-md">
          <CardContent className="pt-6 flex flex-col items-center justify-center text-center space-y-4">
            <Badge
              variant="secondary"
              className="bg-accent/10 text-accent font-medium px-3 py-1 text-sm rounded-full mb-2"
            >
            ...
            <div>
              ...
              <p
                ...
                data-testid="text-gestational-age"
              >
                {formatGestationalAge(
                  results.currentGestationalAgeWeeks,
            ...
            <div className="w-full max-w-md space-y-2 mt-4">
              <div className="flex justify-between text-xs text-muted-foreground font-medium px-1">
                ...
                <span className="text-primary font-semibold">
                  {results.progressPercentage.toFixed(0)}%
                </span>
                <span>Birth</span>
              </div>
              <Progress
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex justify-between items-center py-2 border-b border-border/50">
              ...
              <span className="font-medium text-foreground">
                {format(results.lmp, "MMMM d, yyyy")}
              </span>
            </div>
            <div className="flex justify-between items-center py-2">
              ...
              <span className="font-medium text-foreground">
                {format(results.conception, "MMMM d, yyyy")}
              </span>
            </div>
          </CardContent>
        </Card>
      ...
      {currentWeekData && (
        <div className="space-y-4 mt-2" data-testid="fetal-development-section">
          ...
          <Card
            ...
            data-testid="card-current-week"
          >
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <CardTitle className="text-lg">
                  This Week: Week {currentWeekData.week}
                </CardTitle>
...
function MilestoneItem({ title, date }: { title: string; date: Date }) {
  ...
  return (
    <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active py-3">
      <div
        className={`flex items-center justify-center w-10 h-10 rounded-full border-4 border-card bg-primary/20 shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow-sm z-10 ${completed ? "bg-primary/40 text-primary" : "text-primary/50"}`}
      >
        {completed ? (
      ...
      <div
        className={`w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-xl border ${completed ? "bg-card border-primary/20 shadow-sm" : "bg-muted/30 border-border/50"} `}
      >
        <div className="flex items-center justify-between space-x-2 mb-1">
          <h4
            className={`font-semibold text-sm ${completed ? "text-foreground" : "text-muted-foreground"}`}
          >
            {title}
          </h4>
          {completed && (
        ...
        <p
          className={`text-sm ${completed ? "text-foreground/80" : "text-muted-foreground/70"}`}
        >
