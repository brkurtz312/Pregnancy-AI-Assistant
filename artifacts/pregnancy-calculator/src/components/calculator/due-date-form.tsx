...
import { calculateByDueDate, PregnancyResults } from "@/lib/pregnancy-math";
...
export function DueDateForm({ onCalculate }: DueDateFormProps) {
  ...
  function onSubmit(values: FormValues) {
    const results = calculateByDueDate(values.dueDate);
    onCalculate(results);
  }
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-base text-foreground/80">Expected Due Date (EDC)</FormLabel>
                  <FormControl>
                    <Input 
                      type="date" 
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-2">
              <Button 
                ...
                data-testid="button-calculate-due-date"
              >
                Calculate
              </Button>
              <Button 
                ...
                data-testid="button-reset-due-date"
              >
                Reset
              </Button>
            </div>
